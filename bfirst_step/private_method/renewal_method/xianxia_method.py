...
class run_xianxia:
    ...
    # 线上请求
    def axianxia_methods(self,xianxiadir, chengshi, gongshi, case, query_details_listna):
        ...
        for licenseno in licenseno_open:
            ...
            if xubaoreturn == '续保成功' or xubaoreturn == '获取车辆信息成功(车架号，发动机号，品牌型号及初登日期)，险种获取失败':
                ...
                for Selecttheusec in Select:
                    ...
                    pichulis = run_xianxia.pichuli(self, Selecttheusec)
                    ...
                    if baojia.res["StatusMessage"] == '请求发送成功':
                        ...
                        if Offer_results1 == '获取报价信息成功':
                            ...
                            if QuoteResultt1 > 0:
                                if Repeat_insurance == True:
                                    ...
                                    run_xianxia.baojiareturn(self, Selecttheusec, baojiajg.res['Item'],
                                                                 licenseno)
                                    Xianshang_dandu1 = run_xianxia()
                        ...
                        # 获取报价结果失败
                            ...
                            Xianshang_dandu1 = run_xianxia()
                            Xianshang_dandu1.rizhibaojia()
                            run_xianxia.baojiareturn(self, Selecttheusec, baojiajg.res['Item'], licenseno)
                            continue
                    # 请求发送失败
                    else:
                        Xianshang_dandu1 = run_xianxia()
            ...
            for k, v in datas.items():
                if k == 'QuoteStatus':
                    if k == 'QuoteStatus' and v <= 0:
                        ...
                        duibijieguoshi = '无'
                        baoebaofeiduibi = '失败'
                        views.details_data(licenseno, duibishibai1['保险公司'], duibishibai1['报价城市'], duibishibai1['续保结果'], duibishibai1['报价状态'], duibishibai1['请求用例id'], dictnews, duibijieguoshi, baoebaofeiduibi, query_details)
                    else:
                        ...
                        duibijieguoshi = '无'
                        baoebaofeiduibi = '成功'
                        views.details_data(licenseno, duibichenggong['保险公司'], duibichenggong['报价城市'], duibichenggong['续保结果'], duibichenggong['报价状态'], duibichenggong['请求用例id'], dictnews, duibijieguoshi, baoebaofeiduibi, query_details)
    ...
    def rizhixubao(self):
        ...
        sceneid = '0'
        details = '0'
        views.details_data(licenseno, Insurance, xianxiainfo['报价城市'], xianxiainfo['续保结果'], offer, sceneid, dictnews, duibijieguoshi, baoebaofeiduibi, query_details)
        return xianxiainfo
    def rizhibaojiashibai(self):
        dictnews = json.dumps(xianxiainfo, ensure_ascii=False, sort_keys=True,indent=2)
        #logbug.debug("日志记录--{}--{}--{}".format(licenseno, xianxiainfo['请求用例'], dictnews))
        duibijieguoshi = '无'
        baoebaofeiduibi = '失败'
        offer = '报价失败'
        details = '0'
        views.details_data(licenseno,  xianxiainfo['保险公司'], xianxiainfo['报价城市'], xianxiainfo['续保结果'], offer, xianxiainfo['请求用例id'], dictnews, duibijieguoshi, baoebaofeiduibi, query_details)
        return xianxiainfo

    # iis超时返回前端处理
    def iischaoshi(self):
        errors = '请求超时'
        return redirect('/batch/',{'error': errors})
